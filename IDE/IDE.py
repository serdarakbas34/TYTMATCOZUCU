...
class IDE(wx.Frame):
    ...
    def InitUI(self):
        ...
        menubar = wx.MenuBar()
        filemenu = wx.Menu()
        menunew = filemenu.Append(wx.ID_NEW, "&New", "Yeni bir dosya oluşturunuz.")
        menuopen = filemenu.Append(wx.ID_OPEN, "&Open", "Düzenlemek için bir dosya açınız.")
        menusave = filemenu.Append(wx.ID_SAVE, "&Save", "Geçerli dosyayı kaydediniz.")
        menusaveas = filemenu.Append(wx.ID_ANY, "SaveAs", "Geçerli dosyayı belirli bir dosya adıyla kaydediniz")
        menuabout = filemenu.Append(wx.ID_ABOUT, "&About", "IDE hakkında bilgi")
        menuexit = filemenu.Append(wx.ID_EXIT, "E&xit", "IDE'yi kapatınız.")
        ...
        runmenu = wx.Menu()
        menuinterpret = runmenu.Append(wx.ID_ANY, "&Interpret", "Programı çalıştırmak için python yorumlayıcısını kullanınız.")
        menudebug = runmenu.Append(wx.ID_ANY, "&Debug", "Programı hata ayıklama çıktısıyla çalıştırınız")
        self.Bind(wx.EVT_MENU, self.OnInterpret, menuinterpret)
        self.Bind(wx.EVT_MENU, self.OnDebug, menudebug)
        menubar.Append(runmenu, "&Run")
        settingsmenu = wx.Menu()
        menupython = settingsmenu.Append(wx.ID_ANY, "&Python", "Python ayarlarını yapılandırınız.")
